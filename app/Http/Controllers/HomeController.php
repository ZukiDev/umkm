...
class HomeController extends Controller
{
    ...
    private function getData()
    {
        ...
        $bestUMKM = Store::where('stores.status', 1)
            ...
            ->select('stores.id', 'stores.user_id', 'stores.logo', 'stores.status', 'stores.created_at', 'stores.updated_at', 'stores.deleted_at', DB::raw('SUM(order_details.quantity) as total_quantity'))
            ->groupBy('stores.id', 'stores.user_id', 'stores.logo', 'stores.status', 'stores.created_at', 'stores.updated_at', 'stores.deleted_at')
            ->orderBy('total_quantity', 'desc')
            ->take(6)
            ->get();
